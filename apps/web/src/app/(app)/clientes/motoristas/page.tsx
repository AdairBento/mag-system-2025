  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 flex-wrap">
        <Input type="text" placeholder="Buscar por nome, email, CNH ou CPF..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 min-w-[300px]" />
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge className={motorista.status === 'ativo' ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white'}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-gray-400">
                    ...
                    <div className="col-span-2">Endereco: {motorista.endereco}, {motorista.cidade} - {motorista.estado} {motorista.cep}</div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button onClick={() => handleEdit(motorista)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">Editar</Button>
                  <Button onClick={() => handleDelete(motorista.id)} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">Deletar</Button>
                </div>
              </div>
            </Card>
          ))
      ...
      <Modal isOpen={isFormOpen} onClose={handleCloseForm} title={editingId ? 'Editar Motorista' : 'Novo Motorista'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input type="tel" name="telefone" placeholder="Telefone" value={formData.telefone} onChange={handleInputChange} required />
            <Input type="text" name="cpf" placeholder="CPF" value={formData.cpf} onChange={handleInputChange} required />
            <Input type="text" name="cnh" placeholder="CNH" value={formData.cnh} onChange={handleInputChange} required />
            <Input type="date" name="validadeCNH" placeholder="Validade CNH" value={formData.validadeCNH} onChange={handleInputChange} required />
            <Input type="date" name="dataNascimento" placeholder="Data Nascimento" value={formData.dataNascimento} onChange={handleInputChange} required />
            <Input type="text" name="endereco" placeholder="Endereco" value={formData.endereco} onChange={handleInputChange} required />
            ...
            <Input type="text" name="cep" placeholder="CEP" value={formData.cep} onChange={handleInputChange} required />
          </div>
